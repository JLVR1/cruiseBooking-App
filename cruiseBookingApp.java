...
public class cruiseBookingApp {
    ...
    private void createCruiseSelectionPanel() {
        ...
        String[] cruiseOptions = {
            "Caribbean Cruise (R18,000)",
            "Holiday Cruise (R28,000)",
            "VIP Cruise (R37,000)"
        };
        ...
        addLabel(cruiseSelectionPanel,"Select Date:",cruiseConstraints,3);
        cruiseConstraints.gridx = 1;
        cruiseSelectionPanel.add(dateComboBox,cruiseConstraints);
        ...
        addLabel(cruiseSelectionPanel,"Select Time:",cruiseConstraints,4);
        cruiseConstraints.gridx = 1;
        cruiseSelectionPanel.add(timeComboBox,cruiseConstraints);
        ...
        nextPageButton1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if ("Select Date".equals(selectedDate) || "Select Time".equals(selectedTime)) {
    ...
    private void handleUserSubmission() {
        if (validateForm()) {
            ...
            writeToFile("Date: " + dateComboBox.getSelectedItem());
            writeToFile("Time: " + timeComboBox.getSelectedItem());
    ...
    private boolean validateCardInfo() {
        ...
        Pattern pattern = Pattern.compile(expiryDateRegex);
        Matcher matcher = pattern.matcher(expiryDate);
        

        if (cardNumber.length() == 16 && matcher.matches() && cvv.length() == 3) {
            return true;
        }
    ...
    private void displayConfirmation() {
        String confirmationMessage = "Thank you " + firstNameField.getText() + " " + lastNameField.getText() + ",\n" +
                "You have selected " + selectedCruise + " cruise with the " + selectedSuite + 
                "\nThis cruise will depart on " + dateComboBox.getSelectedItem() + " at " + timeComboBox.getSelectedItem() + ".\n"+
